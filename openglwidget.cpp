...
void OpenGLWidget::drawAxis() {
  ...
  glUniform4f(glGetUniformLocation(shaderProgram, "color"), 1.0f, 0.0f, 0.0f,
              1.0f);

  glDrawArrays(GL_LINES, 0, 2);
  glUniform4f(glGetUniformLocation(shaderProgram, "color"), 0.0f, 1.0f, 0.0f,
              1.0f);
...
void OpenGLWidget::createVBOs() {
  ...
  glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(QVector4D),
  ...
  glBufferData(GL_ARRAY_BUFFER, colors.size() * sizeof(QVector4D),
               colors.data(), GL_STATIC_DRAW);
  glVertexAttribPointer(
      1, 4, GL_FLOAT, GL_FALSE, 0,
      nullptr); // como os dados acima estao organizados para chegar na GPU
  ...
  float v[] = {-1.0f, 0.0f,  0, 1, 1.0f, 0.0f, 0, 1,
               0.0f,  -1.0f, 0, 1, 0.0f, 1.0f, 0, 1};
  glGenBuffers(1, &vboVerticesAxis);
  glBindBuffer(GL_ARRAY_BUFFER, vboVerticesAxis);
  glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(QVector4D), v,
               GL_STATIC_DRAW);
  glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, nullptr);
  glEnableVertexAttribArray(0);
}
...
void OpenGLWidget::changeDiagonal() {
  makeCurrent();
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, eboIndices);
  auto idx{static_cast<GLuint *>(glMapBufferRange(
      GL_ELEMENT_ARRAY_BUFFER, 0, indices.size() * sizeof(GL_UNSIGNED_INT),
      GL_MAP_WRITE_BIT))};
  idx[0] = 0;
  idx[1] = 1;
  idx[2] = 3;
